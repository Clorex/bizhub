...
export const OrderRow = memo(function OrderRow({ order, onClick }: OrderRowProps) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-3 p-4 rounded-xl border border-gray-100 bg-white hover:border-orange-200 hover:shadow-sm transition-all duration-150 text-left active:scale-[0.99]"
    >
      ...
      <div className="flex-1 min-w-0 grid grid-cols-[1fr_auto] gap-3 items-start">
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1.5 flex-wrap min-w-0">
            ...
            <span className="text-micro text-gray-400 truncate max-w-[10rem]">{formatPaymentType(order?.paymentType)}</span>
          </div>
          <div className="flex items-center gap-1.5 mt-1.5 text-gray-400 min-w-0">
            <Clock className="w-3 h-3 shrink-0" />
            <span className="text-micro truncate">{fmtDate(order?.createdAt)}</span>
          </div>
        </div>
        ...
        <div className="text-right shrink-0 min-w-[96px]">
          ...
          <p className="text-micro text-gray-400 mt-1 whitespace-nowrap">
