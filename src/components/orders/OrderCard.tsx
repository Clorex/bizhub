...
function getOrderStatus(o: any): { status: OrderStatus; label: string } {
  ...
  if (ops === "delivered" || orderStatus.includes("delivered")) return { status: "delivered", label: "Delivered" };
  if (ops === "cancelled" || orderStatus.includes("cancel")) return { status: "cancelled", label: "Cancelled" };
  if (ops === "in_transit") return { status: "in_transit", label: "In Transit" };
  if (escrow === "disputed") return { status: "attention", label: "Needs Attention" };
  if (ops === "paid" || orderStatus.includes("paid") || escrow === "released") return { status: "paid", label: "Paid" };
...
const STATUS_CONFIG: Record<OrderStatus, { icon: any; bgColor: string; textColor: string; iconColor: string }> = {
  processing: { icon: Clock, bgColor: "bg-orange-50", textColor: "text-orange-700", iconColor: "text-orange-500" },
  paid: { icon: CheckCircle2, bgColor: "bg-blue-50", textColor: "text-blue-700", iconColor: "text-blue-500" },
  in_transit: { icon: Truck, bgColor: "bg-purple-50", textColor: "text-purple-700", iconColor: "text-purple-500" },
  delivered: { icon: CheckCircle2, bgColor: "bg-green-50", textColor: "text-green-700", iconColor: "text-green-500" },
  cancelled: { icon: XCircle, bgColor: "bg-gray-100", textColor: "text-gray-600", iconColor: "text-gray-400" },
  attention: { icon: AlertCircle, bgColor: "bg-red-50", textColor: "text-red-700", iconColor: "text-red-500" },
};
          ...
          <div className="flex-1 min-w-0">
            <div className="grid grid-cols-[1fr_auto] gap-2 items-start">
              <div className="min-w-0">
                <p className="text-sm font-bold text-gray-900 truncate">Order #{displayNo}</p>
              ...
              <div className="text-right shrink-0 min-w-[96px]">
                <p className="text-base font-bold text-gray-900 tabular-nums whitespace-nowrap">{formatMoneyNGN(amount)}</p>
              </div>
            </div>

            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-3 min-w-0">
              <span className={cn("inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap shrink-0", config.bgColor, config.textColor)}>
                {label}
              </span>

              {paymentLabel && <span className="text-xs text-gray-400 truncate max-w-[10rem]">{paymentLabel}</span>}
            </div>

            {dateStr && <p className="text-xs text-gray-400 mt-2 truncate">{dateStr}</p>}
          </div>
