...
function installmentRulesForPlan(planKey: string) {
  ...
  if (k === "APEX") {
    // "no cap" => use very high safe caps to avoid abuse
...
function cleanInstallments(list: any[], maxInstallments: number) {
  ...
  // build + filter invalid/zero amounts
  const tmp = arr
    .map((x, idx) => {
  ...
  // re-index sequentially (IMPORTANT)
  const out = sliced.map((x, idx) => ({
    idx,
...
function canUseInstallmentsForPaymentType(paymentType: string, orderSource: string) {
  ...
  // marketplace orders are paystack only
  if (src === "market" || src === "marketplace") return pt === "paystack_escrow";

  // store orders: allow paystack escrow + bank transfer
...
function validateInstallmentDates(installments: any[], nowMs: number, maxPlanDays: number) {
  ...
  // Enforce order of due dates (no reordering server-side to avoid confusion)
  ...
  // Rolling window limit: last due date must be within maxPlanDays from now
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ orderId: string }> }) {
  try {
    ...
    // Do not allow changing a plan if payments already started
    ...
    const plan = {
      enabled: true,
      type: "installments",
      tier: rules.tier, // ✅ helpful for debugging/auditing
      totalKobo,
