...
export async function POST(req: Request, ctx: { params: Promise<{ orderId: string; idx: string }> }) {
  try {
    ...
    if (isSettled(inst?.status)) {
      return Response.json({ ok: false, error: "This installment is already completed." }, { status: 400 });
    }
    ...
    if (!phoneOk && !emailOk) {
      return Response.json({ ok: false, error: "Customer details do not match this order." }, { status: 403 });
    }
    ...
    if (!secureUrl || !cloudPublicId) return Response.json({ ok: false, error: "Upload failed" }, { status: 500 });
    ...
    return Response.json({ ok: true, proof: { idx: i, status: "submitted", uploadedAtMs: now, secureUrl } });
  } catch (e: any) {
    if (e?.code === "VENDOR_LOCKED") {
      return Response.json({ ok: false, code: "VENDOR_LOCKED", error: "Store locked." }, { status: 403 });
    }
