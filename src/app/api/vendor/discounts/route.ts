...
function cleanProductIds(v: any): string[] {
  ...
  const out = v.map((x) => String(x || "").trim()).filter(Boolean).slice(0, 5000);
...
function productCapForPlan(planKey: string): number {
  ...
  if (k === "APEX") return 5000; // “unlimited” fair-use cap
  if (k === "MOMENTUM") return 50;
  if (k === "LAUNCH") return 30;
  return 5;
}

function canSchedule(planKey: string) {
  const k = String(planKey || "FREE").toUpperCase();
  return k === "MOMENTUM" || k === "APEX";
}

async function commitBatches(writes: Array<{ ref: FirebaseFirestore.DocumentReference; data: any }>) {
  const CHUNK = 400;
  ...
  try {
    ...
    return NextResponse.json({
      ...
      meta: {
        ...
        scheduledAllowed: canSchedule(planKey),
        hotDealsEligible: planKey === "MOMENTUM" || planKey === "APEX",
      },
      discounts,
    });
  } catch (e: any) {
...
export async function POST(req: Request) {
  try {
    ...
    const saleBoosted = planKey === "MOMENTUM" || planKey === "APEX";
    ...
    for (const pid of toRemove) {
      ...
      writes.push({
        ...
        data: {
          ...
          salePlanKey: FieldValue.delete(),
          saleBoosted: FieldValue.delete(),
