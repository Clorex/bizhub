...
export async function POST(req: Request, ctx: { params: Promise<{ orderId: string }> }) {
  try {
    ...
    if (!access.limits.canUpdateStatus) {
      return Response.json({ ok: false, code: "FEATURE_LOCKED", error: "Upgrade to update order progress." }, { status: 403 });
    }
    ...
    if (!opsStatus) return Response.json({ ok: false, error: "Invalid opsStatus" }, { status: 400 });
    ...
    return Response.json({ ok: true, opsStatus });
  } catch (e: any) {
