...
function fmtRelativeTime(v: any): string {
  try {
    ...
    return date.toLocaleString("en-NG", { month: "short", day: "numeric" });
  } catch {
    return "";
  }
}
...
function getStatusInfo(status: string): { label: string; bg: string; text: string; icon: any; dotColor: string } {
...
export default function VendorOrdersPage() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const ordersRes = await fetch("/api/vendor/orders", { headers: { Authorization: `Bearer ${token}` } });
  ...
  const exportCsv = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch("/api/vendor/orders/export", { headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) {
  ...
  const stats: OrderStats = useMemo(() => {
    ...
    const todayRevenue = orders
      ...
      .reduce((sum, o) => sum + Number(o.amount || (o.amountKobo ? o.amountKobo / 100 : 0) || 0), 0);
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (q) {
      list = list.filter((o) => {
        ...
        const orderNo = padOrderNumber(o.orderNumber).toLowerCase();

        return id.includes(q) || orderNo.includes(q) || status.includes(q) || customerName.includes(q) || customerPhone.includes(q);
      });
    }
      ...
      <div className="px-4 space-y-4 pt-4">
          ...
          <div className="bg-gradient-to-br from-orange-500 via-orange-500 to-orange-600 rounded-3xl p-5 text-white shadow-lg relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-bold uppercase tracking-widest text-orange-100">Today's Summary</span>
              </div>
        ...
        {orders.length > 0 && filteredOrders.length === 0 && (
          <VendorEmptyState
            ...
            actions={[{ label: "Clear Filters", onClick: () => { setFilter("all"); setSearchQuery(""); }, variant: "primary" }]}
          />
        )}
        ...
        {stats.new > 0 && filter !== "new" && (
          ...
          >
            ...
            <div className="flex-1 text-left">
              <p className="text-sm font-bold text-orange-800">{stats.new} new order{stats.new !== 1 ? "s" : ""} waiting</p>
...
function QuickStatButton({
  ...
  const colorStyles = {
    orange: active ? "bg-orange-500 border-orange-500 text-white" : "bg-orange-50 border-orange-200 text-orange-700 hover:border-orange-300",
    blue: active ? "bg-blue-500 border-blue-500 text-white" : "bg-blue-50 border-blue-200 text-blue-700 hover:border-blue-300",
    green: active ? "bg-green-500 border-green-500 text-white" : "bg-green-50 border-green-200 text-green-700 hover:border-green-300",
    red: active ? "bg-red-500 border-red-500 text-white" : "bg-red-50 border-red-200 text-red-700 hover:border-red-300",
  };
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">#{displayNo}</p>
            </div>
          ...
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            ...
            {isNew && <span className="px-2 py-0.5 rounded-full bg-orange-500 text-white text-[10px] font-bold animate-pulse">NEW</span>}
          </div>
        </div>
