...
export async function GET() {
  ...
  const js = `/* myBizHub FCM Service Worker (generated) */
self.__MYBIZHUB_FCM__ = true;
...
try {
  if (!cfg.apiKey || !cfg.projectId || !cfg.messagingSenderId || !cfg.appId) {
    console.warn("[myBizHub] Missing NEXT_PUBLIC firebase config env; push will not work.");
  } else {
    ...
    messaging.onBackgroundMessage((payload) => {
      ...
      const url = data.url || "/vendor/orders";
  ...
  console.warn("[myBizHub] SW init error", e);
}

self.addEventListener("notificationclick", (event) => {
  const url = event?.notification?.data?.url || "/vendor/orders";
