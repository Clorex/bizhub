...
function startOfTodayMsLagos(now = new Date()) {
  // Lagos is UTC+1; using locale date to get correct day boundary
  const dk = now.toLocaleDateString("en-CA", { timeZone: "Africa/Lagos" }); // YYYY-MM-DD
  const [y, m, d] = dk.split("-").map((x) => Number(x));
  // Create as UTC midnight then shift to Lagos midnight? Simplest: treat as local server date pieces.
...
export async function buildBusinessDailySnapshot(args: { businessId: string }) {
  ...
  // Last re-engagement campaign time (quiet, best-effort)
